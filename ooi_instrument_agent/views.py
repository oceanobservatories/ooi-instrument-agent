...
def lockout(func):
    ...
    def inner(driver_id):
        ...
        locker = page.lock_manager.check_lock(driver_id)
...
def get_lock(driver_id):
    return jsonify({'locked-by': page.lock_manager.check_lock(driver_id)})


@page.route('/api/<driver_id>/lock', methods=['POST'])
def set_lock(driver_id):
    key = request.form.get('key')
    return jsonify({'locked-by': page.lock_manager.set_lock(driver_id, key)})


@page.route('/api/<driver_id>/unlock', methods=['POST'])
@page.route('/api/<driver_id>/lock', methods=['DELETE'])
def unlock(driver_id):
    return jsonify({'locked-by': page.lock_manager.del_lock(driver_id)})


@page.route('/api/locks')
def locks():
    return jsonify({'locks': page.lock_manager.get_locks()})
